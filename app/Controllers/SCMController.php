...
class SCMController extends BaseController
{
	...
	public function index($sup = false)
	{
		...
		if (!empty($supname)) {
			...
			if (file_exists(APPPATH.'views/object/'.strtoupper($sup).'_list.php')){
				return view('object/'.strtoupper($sup).'_list', $data);
				
			}else{
				return view('object/obj_list', $data);
			}
		}else{
	...
	public function getAll($sup = false)
	{
		$this->validation->check($sup, 'required|max_length[2]');
 		$response = $data['data'] = array();	
		$db = db_connect();
		$result = $db->query('SELECT obj_id, ocode, oname, concat_ws(", ",a.location, b.location) as location, tel, email, company, tax FROM obj_db AS o LEFT JOIN location AS a ON o.location_id = a.id_location LEFT JOIN location AS b ON a.id_master = b.id_location ORDER BY o.updated_at DESC');
		...
		$this->objgroupModel = new ObjgroupModel();
		$data['ofilter'] = $this->objgroupModel->getFilter($sup);//load from DB
