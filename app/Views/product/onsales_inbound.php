<form id="data-form-onsales" class="pl-3 pr-3">
                ...
                <tbody id="table_body_onsale">
                    ...
                    <tr>
                        ...
                        <td> <select class="form-select valid_days_temp" id="tempSelect" onchange="select_temp()">
                            ...
                            <button class="btn btn-success" type="button" onclick="loadOnsaleData()">Create price table</button>
                        </td>
    ...
    <hr>
    <div class=" blackout_content">
        <h2>Black out dates</h2>

        <div class="col-md-4 blackout_temp_content">
            <select class="form-select" id="blackout_tempSelect" onchange="select_blackout_temp()">
                <option value="0" selected>No blackout</option>
                <?php
                foreach ($blackout_temp as $black_tmp) { ?>
                    <option value="<?= $black_tmp->id ?>"><?= $black_tmp->name ?></option>
                <?php } ?>
            </select>
        </div>
        <table class="table table-bordered table-warning table_onsale_black_out ">
            <thead class="thead_onsale_black_out" id="blackout_tableHead">

            </thead>
            <tbody id="blackout_tableBody">
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="form-group mb-3">
                ...
                <input type="text" id="file_b2b" name="file_b2b" class="form-control form-control-input" placeholder="File b2b" minlength="0" maxlength="250">
            </div>
        </div>
        <div class="col-md-3">
...
<script>
    ...
    function select_blackout_temp() {
        let temp_id = $('#blackout_tempSelect').val();
        let tableHead = $('#blackout_tableHead');
        let tableBody = $('#blackout_tableBody');
        if (temp_id > 0) {
            $.ajax({
                url: '<?= base_url("product/getBlackoutsTemp") ?>', // Đường dẫn tới phương thức AJAX
                type: 'POST',
                data: {
                    temp_id: temp_id
                },
        ...
        <td style="width: 3%;">Action</td>
                    ...
                    $.each(data, function(index, item) {
                        tableBody.append(
                            `<tr>
                      <td><input type="text"  name="blackout_name[]" class="form-control form-control-input" value="${item.blackout_name}"  readonly></td>
                        <td><input type="date" name="blackout_from[]" class="form-control form-control-input"  value="${item.blackout_from}" readonly ></td>
                        <td><input type="date" name="blackout_to[]" class="form-control form-control-input" value="${item.blackout_to}" readonly ></td>
                    <td>
    ...
    $('#addRow').click(function() {
        ...
        var theadEl = $('.table_onsale_black_out thead');

        // Check if the <thead> has any existing <tr> elements
        if (theadEl.find('tr').length === 0) {
            // Create the <thead> content
            var thead = `
    <tr>
        <td style="width: 40%;">Name</td>
        <td>From</td>
